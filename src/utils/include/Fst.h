...
void vb_encode_value(Val n, Cont &out) {
    while (n >= 128) {
...
inline bool get_prefix_length(const std::string &s1, const std::string &s2, size_t &l) {
    l = 0;
...
enum class OutputType { invalid = -1,
                        none_t,
                        uint32_t,
                        uint64_t,
                        string };
...
struct OutputTraits<std::string> {
    ...
    static value_type get_common_prefix(const value_type &a, const value_type &b) {
...
class Dictionary {
public:
    Dictionary(StatePool<output_t> &state_pool)
...
template <typename output_t>
inline void get_common_prefix_and_word_suffix(const output_t &current_output, const output_t &output, output_t &common_prefix, output_t &word_suffix) {
    common_prefix =
...
build_fst_core(const Input &input, Writer &writer, bool need_output) {
    ...
    input([&](const auto &current_word, const auto &_current_output, size_t input_index) {
        ...
        if (need_output) {
            for (auto j = 1u; j <= prefix_length; j++) {
                ...
                get_common_prefix_and_word_suffix(current_output, output, common_prefix, word_suffix);
...
template <typename output_t, typename Input, typename Writer>
inline std::pair<Result, size_t> build_fst(const Input &input, Writer &writer, bool need_output, bool sorted) {
        ...
        [&](const auto &feeder) {
            if (sorted) {
                ...
                std::vector<size_t> sorted_indexes(input.size());
                {
                    ...
                    std::sort(sorted_indexes.begin(), sorted_indexes.end(), [&](const auto &a, const auto &b) {
        ...
        [&](const auto &feeder) {
            if (sorted) {
                ...
                for (const auto &word : input) {
                    if (!feeder(word, static_cast<uint32_t>(input_index), input_index)) {
                        break;
                    }
                    input_index++;
                }
...
union FstOpe {
    ...
    // For char index
    static constexpr size_t char_index_size(bool need_output, bool need_state_output) {
...
template <typename output_t, bool need_state_output>
struct FstRecord {
    FstOpe ope;
    ...
    size_t byte_size() const {
        ...
        if (!ope.data.no_address) {
            sz += vb_encode_value_length(delta);
        }
        if (need_output) {
...
struct FstHeader {
    ...
    FstHeader(OutputType output_type, bool need_state_output, size_t start_address, const std::vector<size_t> &char_index_table)
...
public:
    ...
    FstWriter(std::ostream &os, bool need_output, bool dump, bool verbose, const Input &input)
        : os_(os)
    ...
    ~FstWriter() {
        ...
        FstHeader header(output_type, need_state_output, start_byte_adress, char_index_table_);
    ...
    void write(const State<output_t> &state, char prev_arc) {
        ...
        if (!need_jump_table) {
            ...
            std::sort(&indexes_sorted_by_bigram_count[0], &indexes_sorted_by_bigram_count[arcs.size()], [&](auto i1, auto i2) {
        ...
        for (auto ri = arcs.size(); ri > 0; ri--) {
            ...
            if (!dump_) {
                ...
                if (need_jump_table) {
                    jump_table[i] = accessible_address;
                }
...
private:
    ...
    void initialize_char_index_table(const Input &input) {
        ...
        struct second_order {
            bool operator()(const std::pair<char, size_t> &x, const std::pair<char, size_t> &y) const {
                return x.second < y.second;
            }
        };

        std::priority_queue<std::pair<char, size_t>, std::vector<std::pair<char, size_t>>, second_order>
            que;
...
inline std::pair<Result, size_t> compile(const Input &input, std::ostream &os, bool sorted, bool verbose = false) {
    FstWriter<output_t, true> writer(os, true, false, verbose, [&](const auto &feeder) {
...
template <typename Input>
inline std::pair<Result, size_t> compile(const Input &input, std::ostream &os, bool need_output, bool sorted, bool verbose = false) {
    FstWriter<uint32_t, false> writer(os, need_output, false, verbose, [&](const auto &feeder) {
        for (const auto &word : input) {
            feeder(word);
        }
    });
...
template <typename output_t, typename Input>
inline std::pair<Result, size_t> dump(const Input &input, std::ostream &os, bool sorted, bool verbose = false) {
    FstWriter<output_t, true> writer(os, true, true, verbose, [&](const auto &feeder) {
        for (const auto &[word, _] : input) {
            feeder(word);
        }
    });
...
template <typename Input>
inline std::pair<Result, size_t> dump(const Input &input, std::ostream &os, bool need_output, bool sorted, bool verbose = false) {
    FstWriter<uint32_t, true> writer(os, need_output, true, verbose, [&](const auto &feeder) {
        for (const auto &word : input) {
            feeder(word);
        }
    });
...
template <typename Input>
inline std::pair<Result, size_t> dot(const Input &input, std::ostream &os, bool need_output, bool sorted) {
...
inline OutputType get_output_type(const char *byte_code, size_t byte_code_size) {
    FstHeader header;
    if (!header.read(byte_code, byte_code_size)) {
...
template <typename T>
OutputType get_output_type(const T &byte_code) {
...
inline double cost_replace(std::string_view from, size_t i, std::string_view to, size_t j) {
    auto c1 = from[i];
    auto c2 = to[j];
    ...
    // one char similar sound...
    {
        ...
        for (auto s : similers) {
            if ((c1 == s[0] && c2 == s[1]) || (c1 == s[1] && c2 == s[0]))
                return 0.25;
        }
    }
...
inline bool common_string(std::string_view s1, std::string_view s2, std::string &cs) {
...
inline double jaro_distance(std::string_view s1, std::string_view s2) {
    std::string cs1;
    if (!common_string(s1, s2, cs1))
        return 0;

    std::string cs2;
    if (!common_string(s2, s1, cs2))
        return 0;
...
public:
    ...
    matcher(const char *byte_code, size_t byte_code_size)
        ...
        , byte_code_size_(byte_code_size) {

        if (!header_.read(byte_code, byte_code_size)) {
            return;
        }
...
protected:
    bool match(
        ...
        std::function<void(size_t, const output_t &)> prefixes = nullptr
    ) const {
        ...
        while (i < len) {
            ...
            if (ope.has_jump_table()) {
                ...
                auto found = lower_bound_index(0, jump_table_count, [&](auto i) { return get_arc(i) < ch; });
            ...
            if (trace_) {
                ...
                if (ope.data.has_output) {
                    std::cout << output_suffix;
                }
                std::cout << "\t";

                if (header_.need_state_output) {
                    if (ope.data.has_state_output) {
                        std::cout << state_output;
                    }
                }
                std::cout << "\t";
    ...
    void depth_first_visit(uint32_t address, const std::string &partial_word, const output_t &partial_output, const T &transit, U accept, std::string_view prefix = std::string_view()) const {
        ...
        while (true) {
            ...
            if (ope.data.final) {
                if (atm.is_match()) {
                    if (prefix.empty() || (prefix.size() == 1 && prefix.front() == arc)) {
                        ...
                        accept(word, should_append_state_output ? output + state_output : output);
                    }
                }
            }
            ...
            if (next_address) {
                if (prefix.empty() || prefix.front() == arc) {
                    depth_first_visit(next_address, word, output, atm, accept, prefix.empty() ? prefix : prefix.substr(1));
                }
            }
    ...
    size_t lookup_jump_table(const char *p, size_t index, size_t element_size) const {
        if (element_size == 2) {
    ...
    decltype(auto) suggest_core(std::string_view word, const T &matcher) const {
        using R =
            typename std::conditional<T::has_output, std::tuple<double, std::string, output_t>, std::pair<double, std::string>>::type;
...
public:
    LevenshteinAutomaton(std::string_view sv, size_t max_edits, size_t insert_cost, size_t delete_cost, size_t replace_cost)
        : s_(decode(sv))
    ...
    void step(char c) {
        ...
        for (size_t i = 0; i < state_.size() - 1; i++) {
            ...
            auto edits = std::min({new_state[i] + insert_cost_, state_[i] + cost, state_[i + 1] + delete_cost_});
            new_state.push_back(edits);
        }

        std::transform(new_state.begin(), new_state.end(), state_.begin(), [=](auto edits) { return std::min(edits, max_edits_ + 1); });
    }
...
template <typename output_t>
class map : public matcher<output_t> {
public:
    ...
    output_t at(std::string_view sv) const {
        auto output = output_t{};
        auto ret    = matcher<output_t>::match(sv.data(), sv.size(), [&](const auto &_) { output = _; });
        if (!ret) {
    ...
    bool exact_match_search(std::string_view sv, output_t &output) const {
        return matcher<output_t>::match(sv.data(), sv.size(), [&](const auto &_) { output = _; });
    }
    ...
    bool
    predictive_search(std::string_view sv, std::function<void(const std::string &, const output_t &)> callback) const {
        auto ret = false;
        matcher<output_t>::depth_first_visit(
            matcher<output_t>::header_.start_address, std::string(), output_t{}, DummyAutomaton(), [&](const auto &word, const auto &output) {
                ret = true;
    ...
    std::vector<std::pair<std::string, output_t>>
    edit_distance_search(std::string_view sv, size_t max_edits, size_t insert_cost = 1, size_t delete_cost = 1, size_t replace_cost = 1) const {
        ...
        matcher<output_t>::depth_first_visit(
            matcher<output_t>::header_.start_address, std::string(), output_t{}, LevenshteinAutomaton(sv, max_edits, insert_cost, delete_cost, replace_cost), [&](const auto &word, const auto &output) {
    ...
    void enumerate(T callback) const {
        matcher<output_t>::depth_first_visit(
            matcher<output_t>::header_.start_address, std::string(), output_t{}, DummyAutomaton(), callback
        );
    }
};
...
public:
    ...
    bool common_prefix_search(std::string_view sv, std::function<void(size_t)> prefixes) const {
        return matcher<none_t>::match(
            sv.data(), sv.size(), nullptr, [&](size_t len, const none_t &) { prefixes(len); }
        );
    }
    ...
    bool
    predictive_search(std::string_view sv, std::function<void(const std::string &)> callback) const {
        auto ret = false;
        matcher<none_t>::depth_first_visit(
            matcher<none_t>::header_.start_address, std::string(), none_t{}, DummyAutomaton(), [&](const auto &word, const auto &) {
                ret = true;
                callback(word);
            },
            sv
    ...
    std::vector<std::string> edit_distance_search(std::string_view sv, size_t max_edits, size_t insert_cost = 1, size_t delete_cost = 1, size_t replace_cost = 1) const {
        ...
        matcher<none_t>::depth_first_visit(
            matcher<none_t>::header_.start_address, std::string(), none_t{}, LevenshteinAutomaton(sv, max_edits, insert_cost, delete_cost, replace_cost), [&](const auto &word, const auto &) { ret.emplace_back(word); }
        );
    ...
    template <typename T>
    void enumerate(T callback) const {
        matcher<none_t>::depth_first_visit(matcher<none_t>::header_.start_address, std::string(), none_t{}, DummyAutomaton(), callback);
    }
};
...
inline void decompile(const char *byte_code, size_t byte_code_size, std::ostream &out, bool need_output = true) {
...
template <typename T>
inline void decompile(const T &byte_code, std::ostream &out, bool need_output = true) {
